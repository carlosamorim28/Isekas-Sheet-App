...
const App: React.FC = () => {
  ...
  const handleSaveItem = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newItem: Item = { 
      ...
      attackBonus: itemFormValues.attackBonus,
      damageBonus: itemFormValues.damageBonus,
      relatedSkillName: itemFormValues.relatedSkillName,
      relatedAttr1: itemFormValues.relatedAttr1,
      relatedAttr2: itemFormValues.relatedAttr2
    };

    let newInv = [...activeChar.inventory];
    if (editingItemIndex !== null) {
      newInv[editingItemIndex] = newItem;
    } else {
      newInv.push(newItem);
    }

    updateActiveChar({ ...activeChar, inventory: newInv });
    setShowItemForm(false);
    setEditingItemIndex(null);
    setItemFormValues({
      name: "", type: "weapon", damage: "", defense: 0, description: "", 
  ...
  const handleAttackRoll = (item: Item) => {
    let bonus = item.attackBonus || 0;
    if (item.relatedSkillName) {
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <section className="glass-panel p-6 rounded-2xl border-amber-500/10">
              <div className="flex justify-between items-center mb-6 border-b border-amber-500/10 pb-2">
                ...
                {isEditing && <button onClick={() => { setEditingItemIndex(null); setItemFormValues({ name: "", type: "weapon", damage: "", defense: 0, description: "", attackBonus: 0, damageBonus: 0, relatedSkillName: "", relatedAttr1: "", relatedAttr2: "" }); setShowItemForm(!showItemForm); }} className="bg-indigo-600 text-[10px] px-3 py-1 rounded font-bold transition-transform active:scale-95">{showItemForm ? 'FECHAR' : '+ ITEM'}</button>}
              </div>
              {showItemForm && (
                <form onSubmit={handleSaveItem} className="mb-8 p-6 bg-slate-900/50 rounded-xl border border-amber-500/20 space-y-4 animate-in slide-in-from-top-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col">
                      <label className="text-[7px] text-slate-500 uppercase mb-1">Nome do Item</label>
                      <input value={itemFormValues.name} onChange={e => setItemFormValues({...itemFormValues, name: e.target.value})} placeholder="Ex: Espada Longa" required className="bg-slate-800 p-2 rounded text-xs border border-slate-700 outline-none focus:border-amber-500" />
                    </div>
                    <div className="flex flex-col">
                      <label className="text-[7px] text-slate-500 uppercase mb-1">Tipo</label>
                      <select value={itemFormValues.type} onChange={e => setItemFormValues({...itemFormValues, type: e.target.value as ItemType})} className="bg-slate-800 p-2 rounded text-xs border border-slate-700 outline-none focus:border-amber-500"><option value="weapon">Arma</option><option value="armor">Armadura</option><option value="utility">Utilitário</option></select>
                    </div>
                  </div>
                  ...
                  {itemFormValues.type === 'weapon' && (
                    <div className="space-y-4 p-4 bg-slate-950/40 rounded-lg border border-amber-500/10">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col">
                          <label className="text-[7px] text-slate-500 uppercase mb-1">Dano (Ex: 1d8+2)</label>
                        ...
                        <div className="flex flex-col">
                          <label className="text-[7px] text-slate-500 uppercase mb-1">Bônus Acerto Arma</label>
                          <input type="number" value={itemFormValues.attackBonus} onChange={e => setItemFormValues({...itemFormValues, attackBonus: parseInt(e.target.value) || 0})} className="bg-slate-800 p-2 rounded text-xs border border-slate-700" />
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col">
                          <label className="text-[7px] text-slate-500 uppercase mb-1">Bônus Dano (Fixo)</label>
                          <input type="number" value={itemFormValues.damageBonus} onChange={e => setItemFormValues({...itemFormValues, damageBonus: parseInt(e.target.value) || 0})} className="bg-slate-800 p-2 rounded text-xs border border-slate-700" />
                        </div>
                        <div className="flex flex-col">
                          <label className="text-[7px] text-slate-500 uppercase mb-1">Perícia de Acerto</label>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col">
                          <label className="text-[7px] text-slate-500 uppercase mb-1">Dano: Attr 1</label>
                        ...
                        <div className="flex flex-col">
                          <label className="text-[7px] text-slate-500 uppercase mb-1">Dano: Attr 2</label>
                  ...
                  {itemFormValues.type === 'armor' && (
                    <div className="flex flex-col">
                      <label className="text-[7px] text-slate-500 uppercase mb-1">Defesa (CA)</label>
                      <input type="number" value={itemFormValues.defense} onChange={e => setItemFormValues({...itemFormValues, defense: parseInt(e.target.value) || 0})} placeholder="Bônus CA" className="bg-slate-800 p-2 rounded text-xs border border-slate-700 outline-none focus:border-amber-500" />
                    </div>
                  )}

                  <textarea value={itemFormValues.description} onChange={e => setItemFormValues({...itemFormValues, description: e.target.value})} placeholder="Descrição do item..." className="w-full bg-slate-800 p-2 rounded text-xs h-20 border border-slate-700 outline-none focus:border-amber-500" />
                  <button type="submit" className="w-full bg-green-600 py-2 rounded font-bold text-xs hover:bg-green-500 transition-colors uppercase">{editingItemIndex !== null ? 'Salvar Item' : 'Forjar Item'}</button>
                </form>
              )}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div key={item.id} className={`p-4 rounded-xl bg-slate-800/40 border transition-all ${item.isEquipped ? 'border-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.1)]' : 'border-slate-700/50'} flex flex-col group`}>
                    ...
                    {item.type === 'weapon' && (
                      <div className="flex gap-2 mb-3">
                        <button onClick={() => handleAttackRoll(item)} className="flex-1 bg-amber-900/40 hover:bg-amber-800/60 border border-amber-700/50 text-[10px] py-1.5 rounded-lg font-bold flex flex-col items-center">
                          <span>Ataque</span>
                          <span className="text-[8px] opacity-70">Rolar d20</span>
                        </button>
                        <button onClick={() => handleDamageRoll(item)} className="flex-1 bg-red-900/40 hover:bg-red-800/60 border border-red-700/50 text-[10px] py-1.5 rounded-lg font-bold flex flex-col items-center">
                          <span>Dano</span>
                          <span className="text-[8px] opacity-70">{item.damage || 'N/A'}</span>
                        </button>
                      </div>
                    )}
                    ...
                    {isEditing && (
                      <div className="mt-auto pt-2 flex justify-between items-center border-t border-slate-700/30">
                        <button onClick={() => handleEditItemStart(idx)} className="text-[8px] text-amber-500 uppercase font-bold hover:underline">Editar</button>
